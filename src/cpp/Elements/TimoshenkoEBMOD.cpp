...
//	Constructor
CTimoshenkoEBMOD::CTimoshenkoEBMOD()
{
...
bool CTimoshenkoEBMOD::Read(ifstream& Input, unsigned int Ele, CMaterial* MaterialSets,
                            CNode* NodeList)
{
    unsigned int N;
...
void CTimoshenkoEBMOD::Write(COutputter& output, unsigned int Ele)
{
...
void CTimoshenkoEBMOD::GenerateLocationMatrix()
{
    unsigned int i = 0;
...
//	For 2 node Timoshenko beam element, element stiffness is a 12x12 matrix, whose upper triangular
//part
//	has 78 elements
unsigned int CTimoshenkoEBMOD::SizeOfStiffnessMatrix() { return 78; }

//	Calculate element stiffness matrix
//	Upper triangular matrix, stored as an array column by colum starting from the diagonal element
void CTimoshenkoEBMOD::ElementStiffness(double* Matrix)
{
    ...
    const double by = 12 * Stfy * kMod / G / material->Area / len / len;
    const double bz = 12 * Stfz * kMod / G / material->Area / len / len;
    const double Bendy1 = 12 * Stfy / (1 + by) / len / len / len;
    const double Bendy2 = Bendy1 / 2 * len;
    const double Bendy3 = (4 + by) * Stfy / (1 + by) / len;
    const double Bendy4 = (2 - by) * Stfy / (1 + by) / len;
    const double Bendz1 = 12 * Stfz / (1 + bz) / len / len / len;
    const double Bendz2 = Bendz1 / 2 * len;
    const double Bendz3 = (4 + bz) * Stfz / (1 + bz) / len;
    const double Bendz4 = (2 - bz) * Stfz / (1 + bz) / len;
...
void CTimoshenkoEBMOD::ElementStress(double stress[3], double force[12], double* Displacement)
{
    ...
    stress[1] = G / kMod / len *
                ((EleDisp[7] - EleDisp[1]) - 0.5 * (EleDisp[5] + EleDisp[11]) * len) * bz /
                (1 + bz);
    stress[2] = G / kMod / len *
                ((EleDisp[8] - EleDisp[2]) + 0.5 * (EleDisp[4] + EleDisp[10]) * len) * by /
                (1 + by);
    ...
    force[7] = -force[6];
    force[8] = -Stfy / len / len * (6 * (EleDisp[8] - EleDisp[2]) / (1 + by) -
                                    3 * by / (1 + by) * (EleDisp[4] + EleDisp[10]) * len +
                                    (4 * EleDisp[4] + 2 * EleDisp[10]) * len);
    force[9] = -Stfy / len / len * (6 * (EleDisp[8] - EleDisp[2]) / (1 + by) +
                                    3 * by / (1 + by) * (EleDisp[4] + EleDisp[10]) * len -
                                    (4 * EleDisp[4] + 2 * EleDisp[10]) * len);
    force[10] = Stfz / len / len * (6 * (EleDisp[7] - EleDisp[1]) / (1 + bz) +
                                    3 * bz / (1 + bz) * (EleDisp[5] + EleDisp[11]) * len -
                                    (4 * EleDisp[5] + 2 * EleDisp[11]) * len);
    force[11] = Stfz / len / len * (-6 * (EleDisp[7] - EleDisp[1]) / (1 + bz) -
                                    3 * bz / (1 + bz) * (EleDisp[5] + EleDisp[11]) * len +
                                    (2 * EleDisp[5] + 4 * EleDisp[11]) * len);
}

void CTimoshenkoEBMOD::ElementStress2(double* stress, double* Displacement, double* PrePositions, double* PostPositions)
{
}