...
void CShell::ElementPostInfo(double* stress2, double* Displacement, double* Positions4SE, double* PostPositions)
{
    ...
    for (unsigned int i=0; i<4; ++i){
        stress_nodes[6 * i] = stress[12] - material->h * 0.5 *stress_rates[3 * i];
        stress_nodes[6 * i + 1] = stress[13] -material->h * 0.5 * stress_rates[3* i + 1];
        stress_nodes[6 * i + 2] = stress[14] - material->h * 0.5 * stress_rates[3 * i + 2];
        stress_nodes[6 * i + 3] = 0.0;
        stress_nodes[6 * i + 4] = 0.0;
        stress_nodes[6 * i + 5] = 0.0;
        stress_nodes[6 * i + 24] = stress[12] + material->h * 0.5 *stress_rates[3 * i];
        stress_nodes[6 * i + 25] = stress[13] + material->h * 0.5 * stress_rates[3* i + 1];
        stress_nodes[6 * i + 26] = stress[14] + material->h * 0.5 * stress_rates[3 * i + 2];
