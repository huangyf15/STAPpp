...
bool CTimoshenkoSRINT::Read(ifstream& Input, unsigned int Ele, CMaterial* MaterialSets,
                            CNode* NodeList)
{
    unsigned int N;
...
void CTimoshenkoSRINT::ElementStress(double stress[3], double force[12], double* Displacement)
{
    ...
    stress[1] =
        G / kMod / len * ((EleDisp[7] - EleDisp[1]) - 0.5 * (EleDisp[5] + EleDisp[11]) * len);
    stress[2] =
        G / kMod / len * ((EleDisp[8] - EleDisp[2]) + 0.5 * (EleDisp[4] + EleDisp[10]) * len);
    ...
    force[7] = -force[6];
    force[9] = Stfy / len * (EleDisp[10] - EleDisp[4]) + 0.5 * force[5] * len;
    force[8] = Stfy / len * (EleDisp[10] - EleDisp[4]) - 0.5 * force[5] * len;
    force[11] = Stfz / len * (EleDisp[11] - EleDisp[5]) - 0.5 * force[3] * len;
    force[10] = Stfz / len * (EleDisp[11] - EleDisp[5]) + 0.5 * force[3] * len;
}

void CTimoshenkoSRINT::ElementStress2(double stress[3], double* Displacement, double* PrePositions, double* PostPositions)
{
}
