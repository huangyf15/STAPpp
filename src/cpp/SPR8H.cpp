...
void CHex::ElementPostSPR(double* stressG, double* Displacement, double* PrePositions,
                          double* PostPositions, double* PositionG)
{
    ...
    CHexMaterial* material =
        dynamic_cast<CHexMaterial*>(ElementMaterial); // Pointer to material of the element
    ...
    D << 1, v / (1 - v), v / (1 - v), 0, 0, 0, v / (1 - v), 1, v / (1 - v), 0, 0, 0, v / (1 - v),
        v / (1 - v), 1, 0, 0, 0, 0, 0, 0, (1 - 2 * v) / 2 / (1 - v), 0, 0, 0, 0, 0, 0,
        (1 - 2 * v) / 2 / (1 - v), 0, 0, 0, 0, 0, 0, (1 - 2 * v) / 2 / (1 - v);
    D = D * k;
    ...
    for (unsigned p = 0; p < 8; p++)
    {
        ...
        GN << (1 - eta) * (1 - zeta), (1 + eta) * (1 - zeta), -(1 + eta) * (1 - zeta),
            -(1 - eta) * (1 - zeta), (1 - eta) * (1 + zeta), (1 + eta) * (1 + zeta),
            -(1 + eta) * (1 + zeta), -(1 - eta) * (1 + zeta), -(1 + xi) * (1 - zeta),
            (1 + xi) * (1 - zeta), (1 - xi) * (1 - zeta), -(1 - xi) * (1 - zeta),
            -(1 + xi) * (1 + zeta), (1 + xi) * (1 + zeta), (1 - xi) * (1 + zeta),
            -(1 - xi) * (1 + zeta), -(1 + xi) * (1 - eta), -(1 + xi) * (1 + eta),
        ...
        Be << Bele(0, 0), 0, 0, Bele(0, 1), 0, 0, Bele(0, 2), 0, 0, Bele(0, 3), 0, 0, Bele(0, 4), 0,
            0, Bele(0, 5), 0, 0, Bele(0, 6), 0, 0, Bele(0, 7), 0, 0, 0, Bele(1, 0), 0, 0,
            Bele(1, 1), 0, 0, Bele(1, 2), 0, 0, Bele(1, 3), 0, 0, Bele(1, 4), 0, 0, Bele(1, 5), 0,
            0, Bele(1, 6), 0, 0, Bele(1, 7), 0, 0, 0, Bele(2, 0), 0, 0, Bele(2, 1), 0, 0,
            Bele(2, 2), 0, 0, Bele(2, 3), 0, 0, Bele(2, 4), 0, 0, Bele(2, 5), 0, 0, Bele(2, 6), 0,
            0, Bele(2, 7), Bele(1, 0), Bele(0, 0), 0, Bele(1, 1), Bele(0, 1), 0, Bele(1, 2),
            Bele(0, 2), 0, Bele(1, 3), Bele(0, 3), 0, Bele(1, 4), Bele(0, 4), 0, Bele(1, 5),
            Bele(0, 5), 0, Bele(1, 6), Bele(0, 6), 0, Bele(1, 7), Bele(0, 7), 0, 0, Bele(2, 0),
            Bele(1, 0), 0, Bele(2, 1), Bele(1, 1), 0, Bele(2, 2), Bele(1, 2), 0, Bele(2, 3),
            Bele(1, 3), 0, Bele(2, 4), Bele(1, 4), 0, Bele(2, 5), Bele(1, 5), 0, Bele(2, 6),
            Bele(1, 6), 0, Bele(2, 7), Bele(1, 7), Bele(2, 0), 0, Bele(0, 0), Bele(2, 1), 0,
            Bele(0, 1), Bele(2, 2), 0, Bele(0, 2), Bele(2, 3), 0, Bele(0, 3), Bele(2, 4), 0,
            Bele(0, 4), Bele(2, 5), 0, Bele(0, 5), Bele(2, 6), 0, Bele(0, 6), Bele(2, 7), 0,
            Bele(0, 7);
    ...
    for (unsigned int i = 0; i < 8; i++)
    {
        ...
        eta = eta8[i];
        zeta = zeta8[i];
        N << (1 + xi) * (1 - eta) * (1 - zeta), (1 + xi) * (1 + eta) * (1 - zeta),
               ...
               uint* Ele_NodeNumber, uint NUME, uint NUMNP)
{
            ...
            for (s_uint j = 0; j < 6; j++) //按6个应力分量遍历
            {
                ...
                for (s_uint k = 0; k < NNE[Np]; k++)
                {
                    for (s_uint m = 0; m < 8; m++)
                    {
                        ...
                        p << 1, x, y, z, x * y, x * z, y * z, x * x, y * y, z * z;
                ...
                for (s_uint k = 0; k < N_SPR; k++)
                {
                    ...
                    MatrixXd tmp;
                    tmp = p.transpose() * a;
                    stress_tmp[(NP_SPR[k] - 1) * 6 + j] =
                        stress_tmp[(NP_SPR[k] - 1) * 6 + j] + tmp(0, 0);
                }
            }
        }
    }
    ...
    for (uint Ele = 0; Ele < NUME; Ele++)
    {
        for (s_uint N = 0; N < 8; N++)
        {
            ...
            {
                stress_SPR[Ele * 48 + N * 6 + j] =
                    stress_tmp[(Ele_NodeNumber[Ele * 8 + N] - 1) * 6 + j] /
                    CNT[Ele_NodeNumber[Ele * 8 + N] - 1];
            }
        }
    }
}
