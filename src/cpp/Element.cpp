...
void CElement::CalculateColumnHeight(unsigned int* ColumnHeight)
{
    ...
    for (unsigned int i = 0; i < ND; i++)
    {
        ...
        if (ColumnHeight[column - 1] < Height)
...
void CElement::assembly(double* Matrix, CSkylineMatrix<double>* StiffnessMatrix)
{
    ...
    {
        unsigned int Lj =
            LocationMatrix[j]; // Global equation number corresponding to jth DOF of the element
        if (!Lj)
            continue;

        //		Address of diagonal element of column j in the one dimensional element stiffness
        //matrix
        ...
        {
            unsigned int Li =
                LocationMatrix[i]; // Global equation number corresponding to ith DOF of the element
