...
//!	Constructor
	COutputter2(string FileName2);

//!	Designed as a single instance class
	static COutputter2* _instance2;

public:

//!	Return pointer to the output file stream
	inline ofstream* GetOutputFile2() { return &OutputFile2; }

//!	Return the single instance of the class
	static COutputter2* Instance2(string FileName2 = " ");


//!	Output element stresses 
	void OutputElementStress2();

//! Overload the operator <<
	template <typename T>
	COutputter2& operator<<(const T& item) 
	{
		std::cout << item;
		OutputFile2 << item;
		return *this;
	...
	typedef std::basic_ostream<char, std::char_traits<char> > CharOstream;
	COutputter2& operator<<(CharOstream& (*op)(CharOstream&)) 
	{
		op(std::cout);
		op(OutputFile2);
		return *this;
