...
fileused=fopen('stresses3.txt','wt');
%for i=1:3
%    for j=1:12
...
for i=1:4
    ...
    for j=1:12
        ...
        fprintf(fileused,'%s%20.16f%s%20.16f%s%20.16f%s','(psix*psix+nu*psiy*psiy)*',Bnumeral(j,1),'+(etax*etax+nu*etay*etay)*',Bnumeral(j,2),'+(psix*etax+nu*psiy*etay)*',Bnumeral(j,3),')');
        if(j<=11)
            fprintf(fileused,'%s','+');
        elseif(j==12)
            fprintf(fileused,'%s\n',';');
    ...
    for j=1:12
        fprintf(fileused,'%s%i%s','dis[',j-1,']*(');
        fprintf(fileused,'%s%20.16f%s%20.16f%s%20.16f%s','(nu*psix*psix+psiy*psiy)*',Bnumeral(j,1),'+(nu*etax*etax+etay*etay)*',Bnumeral(j,2),'+(nu*psix*etax+psiy*etay)*',Bnumeral(j,3),')');
        if(j<=11)
            fprintf(fileused,'%s','+');
        elseif(j==12)
            fprintf(fileused,'%s\n',';');
    ...
    for j=1:12
        fprintf(fileused,'%s%i%s','dis[',j-1,']*(');
        fprintf(fileused,'%s%20.16f%s%20.16f%s%20.16f%s','((1-nu)*psix*psiy)*',Bnumeral(j,1),'+((1-nu)*etax*etay)*',Bnumeral(j,2),'+(psix*etax+psiy*etay)*(1-nu)*0.5*',Bnumeral(j,3),')');
        if(j<=11)
            fprintf(fileused,'%s','+');
        elseif(j==12)
            fprintf(fileused,'%s\n',';');
